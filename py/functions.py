...
def universal(img: BuildImage, args: List[str]):
    if not args:
        args = ["在此处添加文字"]
    ...
    def make(img: BuildImage) -> BuildImage:
        ...
        for arg in args:
            text_img = BuildImage(
                Text2Image.from_bbcode_text(arg, fontsize=45, align="center").wrap(480).to_image()
            )
...
def petpet(img: BuildImage, arg: str):
...
def rip(user_imgs: List[BuildImage], ):
    if len(user_imgs) >= 2:
...
def crawl(img: BuildImage, arg: str):
    total_num = 92
...
def littleangel(user: UserInfo, arg: str):
...
def play_game(img: BuildImage, arg: str):
    text = arg or "来玩休闲游戏啊"
...
def back_to_work(img: BuildImage, ):
...
def follow(user: UserInfo, arg: str):
...
def coupon(user: UserInfo, arg: str):
...
def listen_music(img: BuildImage, ):
    img = img.convert("RGBA")
...
async def dianzhongdian(img: BuildImage, arg: str):
    if not arg:
        return REQUIRE_ARG
...
def funny_mirror(img: BuildImage, ):
...
def symmetric(img: BuildImage, arg: str):
...
def safe_sense(user: UserInfo, arg: str):
...
def always_like(users: List[UserInfo], args: List[str]):
...
def interview(imgs: List[BuildImage], arg: str):
    if len(imgs) >= 2:
        self_img = imgs[0]
        user_img = imgs[1]
    else:
...
def decent_kiss(img: BuildImage, ):
...
def anyasuki(img: BuildImage, arg: str):
...
def keepaway(imgs: List[BuildImage], arg: str):
...
def repeat(
        users: List[UserInfo], sender: UserInfo, arg: str
):
...
def anti_kidnap(img: BuildImage, ):
...
def mywife(user: UserInfo, ta: str, name: str):
    ta = ta.strip() or "我"
...
def teach(img: BuildImage, arg: str):
...
def addition(img: BuildImage, arg: str):
